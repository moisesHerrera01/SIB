...
class Solicitud_Disponibilidad extends CI_Controller {
  ...
  public function __construct() {
    ...
    $this->load->model(array('Compras/Solicitud_Compra_Model','Compras/Solicitud_Disponibilidad_Model'));
	}
  ...
  public function mostrarTabla(){
    ...
    if($USER){
      ...
      */
      ...
      $this->table->set_heading('Id','Número','Id disp. financiera', 'Confirmación', 'Sección requisitora','Montos','
        Productos','Confirmar','Denegar','Editar', 'Eliminar', 'Generar');
      ...
      */
      ...
      if ($this->input->is_ajax_request()) {
        if (!($this->input->post('busca') == "")) {
            $registros = $this->Solicitud_Disponibilidad_Model->buscarSolicitudesDisponibilidadLimit($this->input->post('busca'),date("Y"),$USER['rol'],$USER['id_empleado']);
        } else {
            $registros = $this->Solicitud_Disponibilidad_Model->obtenerSolicitudesDisponibilidadLimit($num, $this->uri->segment(4),date("Y"),$USER['rol'],$USER['id_empleado']);
            $pagination = paginacion('index.php/Compras/Solicitud_Disponibilidad/index/', $this->Solicitud_Disponibilidad_Model->totalSolicitudesDisponibilidad(date("Y"),$USER['rol'],$USER['id_empleado']),
                          $num, '4');
        }
      } else {
            $registros = $this->Solicitud_Disponibilidad_Model->obtenerSolicitudesDisponibilidadLimit($num, $this->uri->segment(4),date("Y"),$USER['rol'],$USER['id_empleado']);
      ...
      */
      if (!($registros == FALSE)) {
        foreach($registros as $sol) {
            ...
            'N° requerimientro:$sol->numero_solicitud_compra Fecha:$sol->fecha_solicitud_compra','$sol->nombre_seccion','$sol->id_especifico','$sol->fecha'],
            false,false,false,'observaciones','$sol->observaciones')";
             ...
             if ($USER['rol'] == 'JEFE COMPRAS' || $USER['rol'] == 'JEFE UACI' || $USER['rol'] == 'ADMINISTRADOR SICBAF'){
               if($sol->estado_solicitud_compra=='APROBADA COMPRAS' || $sol->nivel_solicitud==4){
                  ...
                  $eliminar = '<a class="icono icon-eliminar" uri='.base_url('index.php/Compras/Solicitud_Disponibilidad/EliminarDato/'.$sol->id_solicitud_disponibilidad).'></a>';
            ...
            $this->table->add_row($sol->id_solicitud_compra,$numero,$sol->id_solicitud_disponibilidad,$sol->fecha,
            $sol->nombre_seccion, $montos,$botones,$aprobar,$denegar, $actualizar, $eliminar, $solicitud_imp);
        }
      } else {
        $msg = array('data' => "No hay solicitudes de disponibilidad financiera ingresadas", 'colspan' => "12");
  ...
  public function EliminarDato(){
    ...
    if($USER){
      if ($this->User_model->validarAccesoCrud($modulo, $USER['id'], 'delete')) {
        ...
        if ($this->Solicitud_Disponibilidad_Model->existeSolicitudMonto($id)){
          redirect('/Compras/Solicitud_Disponibilidad/index/dependencia');
        } else {
  ...
  public function Aprobar() {
    ...
    if($USER){
      if ($this->Solicitud_Disponibilidad_Model->existeSolicitudMonto($id)) {
        ...
        $this->notificacion->NotificacionSolicitudCompra($id, $USER, 5);
      }else {
        redirect('/Compras/Solicitud_Disponibilidad/index/nomonto');
      }
      redirect('/Compras/Solicitud_Disponibilidad/index/update');
    } else {
      redirect('login/index/error_no_autenticado');
      ...
      if ($registros == '') {
        ...
        foreach ($registros as $sol) {
          ...
          if ($rast) {
            ...
            $sol->id_solicitud_compra.'" data="'.$sol->id_solicitud_compra.'"  data1="N° requerimiento:'.$sol->numero_solicitud_compra.' Fecha:'.$sol->fecha_solicitud_compra.'"
             data2="'.$sol->nombre_seccion.'" data3="'.$sol->id_especifico.'" data4="'.$rast->fecha.'">'
            .'N° requerimiento:'.$sol->numero_solicitud_compra.' Fecha:'.$sol->fecha_solicitud_compra.'</a></div>';
            $i++;
          }
        }
      }
  ...
  public function AutocompleteDisponibilidad() {
    ...
    if($USER){
      ...
      if ($registros == '') {
        ...
        foreach ($registros as $sol) {
          //if ($this->Solicitud_Disponibilidad_Model->verificarOrden($sol->id_detalle_solicitud_disponibilidad)) {
            ...
            data1="'.'Id monto:'.$sol->id_detalle_solicitud_disponibilidad.'|N° req:'.$sol->numero_solicitud_compra.'|Linea:'.$sol->linea_trabajo.'|Monto:$'.$sol->monto_sub_total.'"
            data2="'.$sol->id_solicitud_compra.'" >'
            .'Id monto:'.$sol->id_detalle_solicitud_disponibilidad.'|N° req:'.$sol->numero_solicitud_compra.'|Linea:'.$sol->linea_trabajo.'|Monto:$'.$sol->monto_sub_total.'</a></div>';
            $i++;
          //}
        }if ($i<2) {
          echo '<div id="1" class="suggest-element"><a id="conteo">No se encontraron coincidencias</a></div>';
