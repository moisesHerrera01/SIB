...
class Reporte_orden_compra extends CI_Controller {
  ...
  public function reporte(){
    ...
    if($USER){
      ...
      if ($this->uri->segment(5)!=NULL) {
        ...
        $this->table->set_heading('Nº','Nº Orden','Nº Req','Fecha', 'Folio','Solicitante', 'Descripción', 'Proveedor','Tipo Empresa','Monto');
        $num = '10';
        if ($this->uri->segment(5)==1){
          $tipo='MICRO_EMPRESA';
        } elseif ($this->uri->segment(5)==2) {
          $tipo='PEQUEÑA_EMPRESA';
        } elseif ($this->uri->segment(5)==3) {
          $tipo='MEDIANA_EMPRESA';
        } elseif ($this->uri->segment(5)==4) {
          $tipo='GRAN_EMPRESA';
        } elseif ($this->uri->segment(5)==5) {
          $tipo='OTROS_CONTRIBUYENTES';
        ...
        if (!($registros == FALSE)) {
          ...
          foreach($registros as $orden) {
            ...
            $this->table->add_row($i,$orden->numero_orden_compra.'/'.substr($orden->fecha_solicitud_compra,0,-6), $numero,
             $orden->fecha, $orden->folio, $orden->nombre_seccion, $descripcion.' - '.$orden->justificacion, $orden->nombre_proveedor,$orden->tipo_empresa,'$'.number_format($orden->monto_total_oc,2));
             $monto_total+=$orden->monto_total_oc;
             $i++;
          }
          $tot = array('data' => "TOTAL:", 'colspan' => "9");
        ...
        $table =  "<div class='content_table '>" .
                  ...
                  Exportar Excel</a>&nbsp;
                  <a href='".base_url('/index.php/Compras/Reportes/Reporte_orden_compra/ImprimirReporte/'.$this->uri->segment(5).'/'.$this->uri->segment(6).'/'.
  ...
  public function ImprimirReporte() {
    ...
    if ($USER) {
      if ($this->uri->segment(5) != NULL) {
        ...
        $fecha_inicio=date("d-m-Y",strtotime($this->uri->segment(6)));
        $fecha_fin=date("d-m-Y",strtotime($this->uri->segment(7)));

        $data = array(
          'table' => $this->table->generate(),
          'title' => '1 - Reporte de ordenes de compra.',
          'mtps' => 'MINISTERIO DE TRABAJO Y PREVISIÓN SOCIAL',
          'uaci' => 'UNIDAD DE ADQUISICIONES Y CONTRATACIONES INSTITUCIONAL',
          'param' => "CUADRO DE REPORTE DE ORDENES DE COMPRAS".' DEL '.$fecha_inicio.' AL '.$fecha_fin
        );
  ...
  public function ReporteExcel() {
    ...
    $objPHPExcel->setActiveSheetIndex(0)
                 ...
                 ->setCellValue('E6', 'Folio')
                 ->setCellValue('F6', 'Solicitante')
                 ->setCellValue('G6', 'Descripción')
                 ->setCellValue('H6', 'Proveedor')
                 ->setCellValue('I6', 'Tipo Empresa')
                 ->setCellValue('J6', 'Monto');
    $objPHPExcel->getActiveSheet()->getStyle('A6:J6')->applyFromArray($estilo_titulo);
    ...
    if ($contador > 0) {
      ...
      $objPHPExcel->getActiveSheet()->getStyle('A1:J1')->applyFromArray($estilo_contenido);
      $objPHPExcel->getActiveSheet()->getStyle('A2:J2')->applyFromArray($estilo_contenido);
      $objPHPExcel->getActiveSheet()->getStyle('A4:J4')->applyFromArray($estilo_titulo);

      while ($registro = current($registros)) {
        $descripcion=$this->Orden_Compra_Model->obtenerDescripcionProductos($registro->id_orden_compra);
        $numero;
        if ($registro->nivel_solicitud<4) {
          $numero='S/E';
        }else {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('E'.$i, $registro->folio)
                    ->setCellValue('F'.$i, $registro->nombre_seccion)
                    ->setCellValue('G'.$i, $descripcion.' - '.$registro->justificacion)
                    ->setCellValue('H'.$i, $registro->nombre_proveedor)
                    ->setCellValue('I'.$i, $registro->tipo_empresa)
                    ->setCellValue('J'.$i, '$'.number_format($registro->monto_total_oc,2));
        $objPHPExcel->getActiveSheet()->getStyle('A'.$i.':J'.$i)->applyFromArray($estilo_contenido);
      ...
      $objPHPExcel->setActiveSheetIndex(0)
                  ...
                  ->setCellValue('J'.$i, '$'.number_format($total,2));
      $objPHPExcel->getActiveSheet()->getStyle('A'.$i.':J'.$i)->applyFromArray($estilo_titulo);
    } else {
      $objPHPExcel->setActiveSheetIndex(0)->mergeCells('A2:J2');
