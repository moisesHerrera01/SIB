    ...
    if($USER){
      ...
      if ($this->uri->segment(5)!=NULL) {
        ...
        $num = '10';
          ...
          $pagination = paginacion('index.php/Compras/Reportes/Reporte_linea_presupuestaria/reporte/'.$this->uri->segment(5).'/'.$this->uri->segment(6).'/'.$this->uri->segment(7),$total, $num, '8');
          $descripcion='';
          $monto_total=0.0;
          $linea='';
        if (!($registros == FALSE)) {
          ...
          foreach($registros as $orden) {
            ...
            $this->table->add_row($i,$orden->numero_orden_compra.'/'.substr($orden->fecha_solicitud_compra,0,-6),
             $orden->nombre_fuente.'-'.$orden->numero_solicitud_compra.'/'.substr($orden->fecha_solicitud_compra,0,-6),$orden->fecha,
             $orden->nombre_seccion, $descripcion.'-'.$orden->justificacion, $orden->nombre_proveedor,
             '$'.number_format($orden->monto_total_oc,2));
        ...
        $table =  "<div class='content_table '>" .
                  ...
                  Exportar Excel</a>&nbsp;
                  <a href='".base_url('/index.php/Compras/Reportes/Reporte_linea_presupuestaria/ImprimirReporte/'.$this->uri->segment(5).'/'.$this->uri->segment(6).'/'.
    ...
    if ($USER) {
      if ($this->uri->segment(5) != NULL) {
        ...
        $data = array(
          ...
          'title' => '2 - Reporte por linea presupuestaria.',
          'mtps' => 'MINISTERIO DE TRABAJO Y PREVISIÓN SOCIAL',
          'uaci' => 'UNIDAD DE ADQUISICIONES Y CONTRATACIONES INSTITUCIONAL',
          'param' => "CUADRO DE REPORTE POR LINEA PRESUPUESTARIA".' DEL '.$fecha_inicio.' AL '.$fecha_fin.' LINEA PRESUPUESTARIA/TRABAJO:'.$linea
        );
    ...
    if ($total_registros> 0) {
      ...
      while ($registro = current($registros)) {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('F'.$i, $descripcion.' '.$registro->justificacion)
                    ->setCellValue('G'.$i, $registro->nombre_proveedor)
                    ->setCellValue('H'.$i, '$'.number_format($registro->monto_total_oc,2));
