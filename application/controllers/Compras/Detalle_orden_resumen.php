     ...
     foreach($registros as $det) {
       ...
       foreach ($datos as $detsol) {
         ...
         $onClick = "llenarFormulario('solicitud', ['id_detalle_orden_resumen','producto', 'autocomplete1', 'cantidad'],
                     [$det->id_detalle_orden_resumen, '$det->id_detalleproducto', '$detsol->producto','$det->cantidad'],
   ...
   if($USER){
     ...
     if ($registros == '') {
       ...
       foreach ($registros as $producto) {
           echo '<div id="'.$i.'" class="suggest-element" ida="producto'.$producto->id_detalle_solicitud_compra.'"><a id="producto'.
           $producto->id_detalle_solicitud_compra.'" data="'.$producto->id_detalleproducto.'"  data1="'.$producto->id_especifico.' - '.$producto->nombre.' - '.$producto->nombre_unidad.' - '.$producto->especificaciones.'"
           data2="'.$producto->cantidad.'" data3="'.$producto->especificaciones.'">'
           .$producto->id_especifico.' - '.$producto->nombre.' - '.$producto->nombre_unidad.' - '.$producto->especificaciones.'</a></div>';
           $i++;
       }
     }
   } else {
