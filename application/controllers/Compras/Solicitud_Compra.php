  ...
  public function mostrarTabla(){
    ...
    if($USER){
      ...
      */
      ...
      $this->table->set_heading('Id','Número', 'Fecha','Solicitante', 'Estado', 'Enviar','Detalle','Imprimir Solicitud','Eliminar','Editar','Estatus');
      ...
      */
      ...
      if ($this->input->is_ajax_request()) {
        if (!($this->input->post('busca') == "")) {
            $registros = $this->Solicitud_Compra_Model->buscarSolicitudesCompraUser($seccion, $this->input->post('busca'),date("Y"));
        } else {
            $registros = $this->Solicitud_Compra_Model->obtenerSolicitudesCompraUserLimit($seccion, $num, $this->uri->segment(4),date("Y"));
            $pagination = paginacion('index.php/Compras/Solicitud_Compra/index/', $this->Solicitud_Compra_Model->totalSolicitudesCompra($seccion,date("Y"))->cantidad,
                          $num, '4');
        }
      } else {
            $registros = $this->Solicitud_Compra_Model->obtenerSolicitudesCompraUserLimit($seccion ,$num, $this->uri->segment(4),date("Y"));
      ...
      */
      if (!($registros == FALSE)) {
        foreach($registros as $sol) {
            ...
            $onClick = "llenarFormulario('solicitud', ['id','fuentes','autocomplete4','fecha','autocomplete1','cargo','linea', 'sol','autocomplete2','cargo_auto','dep_auto','auto','autocomplete3',
            'cargo_admin','dep_admin','admin','valor','forma','lugar','file_name'],
            [$sol->id_solicitud_compra,'$sol->id_fuentes','$sol->nombre_fuente','$sol->fecha_solicitud_compra','$soli->nombre_empleado','$soli->cargo_funcional','$soli->seccion_padre','$soli->id_empleado',
            ...
            '$adm->id_empleado','$sol->precio_estimado','$sol->forma_entrega','$sol->lugar_entrega','$sol->documento_especificaciones'],
            ...
            if($sol->estado_solicitud_compra=='INGRESADA'){
              if ($this->Solicitud_Compra_Model->existeSolicitudCompra($sol->id_solicitud_compra)) {
                ...
                $solicitud_imp='<a class="icono icon-denegar"></a>';
              }
            ...
            $this->table->add_row($sol->id_solicitud_compra,$numero ,$sol->fecha_solicitud_compra,$sol->primer_nombre.' '.$sol->primer_apellido,
             $sol->estado_solicitud_compra, $enviar,$botones,$solicitud_imp,$eliminar,$actualizar,$estatus);
        }
      } else {
        $msg = array('data' => "No hay requerimientos ingresados", 'colspan' => "11");
  ...
  */
  public function RecibirDatos(){
    ...
    if($USER){
      ...
      $data = array(
          ...
          'justificacion' => str_replace($simbolo, $htmlcode, $this->input->post('justificacion')),
  ...
  public function EnviarDato() {
        ...
        if ($this->Solicitud_Compra_Model->existeSolicitudCompra($id)){
          ...
          $this->notificacion->NotificacionSolicitudCompra($id, $this->session->userdata('logged_in'), 1);
