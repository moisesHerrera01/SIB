...
class Productos extends CI_Controller {
  ...
  public function index(){
    ...
    $msg = array('alert' => $this->uri->segment(4), 'controller'=> 'Productos');
  ...
  public function mostrarTabla(){
    ...
    if($USER){
      ...
      */
      ...
      $this->table->set_heading('#','Nombre', 'U.M', 'Nom. generico', 'Estado', 'Caducidad',
      ...
      */
      ...
      if (!($registros == FALSE)) {
        foreach($registros as $producto) {
            ...
            $onClick = "llenarFormulario('Producto', ['id', 'nombre', 'unidadMedida', 'autocomplete','descripcion', 'fecha', 'stok','generico'],
                        [$producto->id_producto, '$producto->nombre', '$producto->id_unidad_medida', '$nmUnidad','$producto->descripcion',
                        '$producto->fecha_caducidad', '$producto->stock_minimo','$producto->generico'], 'estado', '$producto->estado')";

            $this->table->add_row($producto->id_producto, $producto->nombre, $nmUnidad, $producto->generico, $producto->estado, $producto->fecha_caducidad, $producto->stock_minimo,
  ...
  public function EliminarDato(){
    ...
    if($USER){
      if ($this->User_model->validarAccesoCrud($modulo, $USER['id'], 'delete')) {
        ...
        if ($this->Producto->estaAsociado($id)) {
          redirect('/Bodega/Productos/index/asociado');
        }else {
