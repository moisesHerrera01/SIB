...
class Bienes_muebles extends CI_Controller {
  ...
  public function index(){
    ...
    $data['js'] = "assets/js/validate/bienemueble.js";
    ...
    */
    ...
    if($USER){
      if ($this->User_model->validarAccesoCrud($modulo, $USER['id'], 'select')) {
        ...
        if ($USER['rol']=='ADMINISTRADOR SICBAF' || $USER['rol']=='TECNICO UDT') {
          $this->table->set_heading('Id Bien','Descripción', 'Código','Código Anterior','Marca','Modelo',
          'Serie','Precio Unitario','Oficina','Empleado','Editar','Eliminar','Editar Dato Comun','UDT');
        }else {
          $this->table->set_heading('Id Bien','Descripción', 'Código','Código Anterior','Marca','Modelo',
          'Serie','Precio Unitario','Oficina','Empleado','Editar','Eliminar','Editar Dato Comun');
        }
        ...
        */
        ...
        if ($this->input->is_ajax_request()) {
          if (!($this->input->post('busca') == "")) {
              $registros = $this->Bienes_Muebles_Model->buscarBienes_muebles($USER['rol'],$this->input->post('busca'));
          } else {
              $registros = $this->Bienes_Muebles_Model->obtenerBienes_mueblesLimit($num, $this->uri->segment(4),$USER['rol']);
              $pagination = paginacion('index.php/ActivoFijo/Bienes_muebles/index/', $this->Bienes_Muebles_Model->totalBienes_muebles($USER['rol'])->total,
                            $num, '4');
          }
        } else {
              $registros = $this->Bienes_Muebles_Model->obtenerBienes_mueblesLimit($num, $this->uri->segment(4),$USER['rol']);
          ...
          foreach($registros as $bien) {
            ...
            'empleado','autocomplete4'],
              ...
              if ($USER['rol']=='ADMINISTRADOR SICBAF' || $USER['rol']=='TECNICO UDT') {
                ...
                '<a class="icono icon-actualizar" href="'.base_url('index.php/ActivoFijo/Datos_comunes/index/upgrade/'.$bien->id_dato_comun.'/'.$bien->id_bien).'"></a>',
                '<a class="icono icon-display" href="'.base_url('index.php/ActivoFijo/Equipo_informatico/index/'.$bien->id_bien.'/').'"></a>');
              }else {
                $this->table->add_row($bien->id_bien,$bien->descripcion, $bien->codigo,$bien->codigo_anterior,
                $bien->nombre_marca,$bien->modelo,$bien->serie,$bien->precio_unitario,$bien->nombre_oficina,
                $bien->primer_nombre.' '.$bien->primer_apellido,'<a class="icono icon-actualizar" onClick="'.$onClick.'"></a>',
    ...
    if($USER){
      ...
      if (!($this->input->post('id') == '')){
        if ($this->User_model->validarAccesoCrud($modulo, $USER['id'], 'update')) {
          $this->Bienes_Muebles_Model->actualizarBien_mueble($this->input->post('id'),$data);
      ...
      if ($this->User_model->validarAccesoCrud($modulo, $USER['id'], 'insert')) {
        ...
        $cat=$this->Bienes_inmuebles_model->calcularCodigo($this->input->post('dato_comun'));
        $codigo=$cat->numero_categoria.'.'.sprintf("%'06d", $cor).'.'.$cat->numero_subcategoria;

        $data['id_oficina'] = $this->input->post('oficina');
        $data['id_empleado'] = $this->input->post('empleado');
        $data['correlativo'] = $cor;
        $data['codigo'] = $codigo;
