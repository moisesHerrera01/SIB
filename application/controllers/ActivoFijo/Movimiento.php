        ...
        */
        ...
        if ($this->input->is_ajax_request()) {
          if (!($this->input->post('busca') == "")) {
              ...
              $pagination = paginacion('index.php/ActivoFijo/Movimiento/index/', $this->Movimiento_Model->totalMovimiento(),
                            $num, '4');
          }
        } else {
              $registros = $this->Movimiento_Model->obtenerMovimientosLimit($num, $this->uri->segment(4));
...
//Metodo autocomplete para el tipo de movimiento
  ...
  public function cerrar(){
      $id_movimiento=$this->uri->segment(4);
      $this->Movimiento_Model->cerrar($id_movimiento);
      $mov=$this->Movimiento_Model->obtenerTodoMovimiento($id_movimiento);
      $bienes=$this->Movimiento_Model->obtenerBienesMov($id_movimiento);
      $nombre_movimiento=$mov['nombre_movimiento'];
      $condicion='';
      switch ($nombre_movimiento) {
        case 'ROBO/HURTO':
          $condicion='NO UBICADO';
          break;
        case 'BIENES NO UBICADOS':
          $condicion='NO UBICADO';
          break;
        case 'RE-ASIGNACION':
          $condicion='EN USO';
          break;
        case 'DESCARGO Y DESALOJO DEFINITIVO':
          $condicion='INSERVIBLE';
          break;
        case 'EVENTO TEMPORAL FUERA INSTITUCION':
          $condicion='SALIDA EVENTO ESPECIAL';
          break;
        case 'DESCARGO':
          $condicion='DESCARGO';
          break;
        case 'ASIGNACION 1A.VEZ':
          $condicion='EN USO';
          break;
        case 'REGISTRO 1A. VEZ':
          $condicion='EN USO';
          break;
        default:
          break;
      ...
      if ($condicion!=NULL) {
        $con=$this->Movimiento_Model->obtenerCondicionBien($condicion);
        foreach ($bienes as $bien) {
          $this->Bienes_Muebles_Model->actualizarBien_mueble_Cerrar($bien->id_bien,$con['id_condicion_bien']);
        }
      }
