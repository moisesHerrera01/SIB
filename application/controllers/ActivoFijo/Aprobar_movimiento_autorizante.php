...
class Aprobar_movimiento_autorizante extends CI_Controller {
  ...
  public function index(){
    if($this->session->userdata('logged_in')){
      ...
      $msg = array('alert' => $this->uri->segment(4),'fecha'=>$fecha_actual,'solicitante'=>$solicitante,'controller'=>'Solicitud_movimiento');
  		...
  		$data['body'] = $this->load->view('mensajes', $msg, TRUE) . $this->load->view('ActivoFijo/Aprobar_movimiento_autorizante_view',$msg,TRUE) .
      ...
      */
      ...
      if ($this->input->is_ajax_request()) {
        if (!($this->input->post('busca') == "")) {
            $registros = $this->Movimiento_Model->buscarMovimientosAuto($this->input->post('busca'));
        } else {
            $registros = $this->Movimiento_Model->obtenerSolMovimientosAutoLimit($num, $this->uri->segment(4));
            $pagination = paginacion('index.php/ActivoFijo/Aprobar_movimiento_autorizante/index/', $this->Movimiento_Model->totalMovimientosAuto()->total,$num, '4');
        }
      } else {
            $registros = $this->Movimiento_Model->obtenerSolMovimientosAutoLimit($num, $this->uri->segment(4));
            $pagination = paginacion('index.php/ActivoFijo/Aprobar_movimiento_autorizante/index/', $this->Movimiento_Model->totalMovimientosAuto()->total,$num, '4');
      ...
      if (!($registros == FALSE)) {
        foreach($registros as $sol) {
          ...
          $rec=$this->Movimiento_Model->obtenerNombreOficinas($sol->id_oficina_recibe);
            ...
            $botones='<a class="icono icon-detalle" href="'.base_url('index.php/ActivoFijo/Detalle_aprobar_movimiento_autorizante/index/'.$sol->id_movimiento.'/').'"></a>';
            if($USER['rol']=='DIRECTOR ADMINISTRATIVO' || $USER['rol']=='ADMINISTRADOR SICBAF'){
              if($sol->id_tipo_movimiento==7){
                if($sol->nivel_solicitud==1){
                    $actualizar = '<a class="icono icon-actualizar" title="Actualizar" onClick="'.$onClick.'"></a>';
                    $eliminar='<a class="icono icon-eliminar" uri='.base_url('index.php/ActivoFijo/Aprobar_movimiento_autorizante/EliminarDato/'.$sol->id_movimiento).'></a>';
                    $aprobar = '<a class="icono icon-liquidar" href="'.base_url('index.php/ActivoFijo/Aprobar_movimiento_autorizante/Aprobar/'.$sol->id_movimiento.'/').'"></a>';
                    $denegar = '<a class="icono icon-cross" href="'.base_url('index.php/ActivoFijo/Aprobar_movimiento_autorizante/Denegar/'.$sol->id_movimiento.'/').'"></a>';
  ...
  */
  ...
  public function Aprobar() {
    ...
    if($USER){
      ...
      redirect('/ActivoFijo/Aprobar_movimiento_autorizante/index/update');
    } else {
      redirect('login/index/error_no_autenticado');
    }
  ...
  public function Denegar() {
    ...
    if($USER){
      $id = $this->uri->segment(4);
      $nivel = $this->Movimiento_Model->retornarNivel($id)->nivel_solicitud;
      $data = array(
  ...
  public function ActualizarDatos() {
    $modulo=$this->User_model->obtenerModulo('ActivoFijo/Aprobar_movimiento_autorizante');
    $USER = $this->session->userdata('logged_in');
    date_default_timezone_set('America/El_Salvador');
    $anyo=20;
    $fecha_actual=date($anyo."y-m-d");
    ...
    if($USER){
      ...
      if (!($this->input->post('id') == '')){
        if ($this->User_model->validarAccesoCrud($modulo, $USER['id'], 'update')) {
          ...
          redirect('/ActivoFijo/Aprobar_movimiento_autorizante/index/forbidden');
        }
      }
    } else {
      redirect('login/index/error_no_autenticado');
    ...
    if($USER){
      if ($this->User_model->validarAccesoCrud($modulo, $USER['id'], 'delete')) {
        ...
        if ($this->Movimiento_Model->contieneDetalleMovimiento($id)->asociados>0){
          redirect('/ActivoFijo/Aprobar_movimiento_autorizante/index/existeSol');
        }
        else {
          $this->User_model->insertarRastreabilidad($rastrea);
          $this->Movimiento_Model->eliminarMovimiento($id);
          redirect('/ActivoFijo/Aprobar_movimiento_autorizante/index/delete');
        }
      } else {
        redirect('/ActivoFijo/Aprobar_movimiento_autorizante/index/forbidden');
      }
