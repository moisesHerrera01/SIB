      ...
      */
      ...
      if ($this->input->is_ajax_request()) {
        if (!($this->input->post('busca') == "")) {
            $registros = $this->Movimiento_Model->buscarMovimientosAF($this->input->post('busca'));
        } else {
            $registros = $this->Movimiento_Model->obtenerSolMovimientosTotalLimit($num, $this->uri->segment(4));
            $pagination = paginacion('index.php/ActivoFijo/Gestionar_movimiento/index/', $this->Movimiento_Model->totalMovimientosAF()->total,$num, '4');
        }
      } else {
            $registros = $this->Movimiento_Model->obtenerSolMovimientosTotalLimit($num, $this->uri->segment(4));
            $pagination = paginacion('index.php/ActivoFijo/Gestionar_movimiento/index/', $this->Movimiento_Model->totalMovimientosAF()->total,$num, '4');
