      ...
      */
      ...
      if ($this->input->is_ajax_request()) {
        if (!($this->input->post('busca') == "")) {
            $registros = $this->Movimiento_Model->buscarMovimientos($this->input->post('busca'),$USER['id_seccion']);
        } else {
            $registros = $this->Movimiento_Model->obtenerSolMovimientosLimit($num, $this->uri->segment(4), $USER['id_seccion']);
            $pagination = paginacion('index.php/ActivoFijo/Aprobar_movimiento/index/', $this->Movimiento_Model->totalMovimientos($USER['id_seccion'])->total,$num, '4');
        }
      } else {
            $registros = $this->Movimiento_Model->obtenerSolMovimientosLimit($num, $this->uri->segment(4),$USER['id_seccion']);
            $pagination = paginacion('index.php/ActivoFijo/Aprobar_movimiento/index/',$this->Movimiento_Model->totalMovimientos($USER['id_seccion'])->total,$num,'4');
        ...
        foreach($registros as $sol) {
          ...
          $rec=$this->Movimiento_Model->obtenerNombreOficinas($sol->id_oficina_recibe);
            ...
            if ($sol->id_tipo_movimiento!=7){
              if ($USER['rol']=='COLABORADOR BODEGA' || $USER['rol']=='JEFE BODEGA' || $USER['rol']=='JEFE UNIDAD' ||
                  $USER['rol']=='JEFE COMPRAS' || $USER['rol']=='JEFE UACI' || $USER['rol']=='JEFE AF' ||
                  $USER['rol']=='COLABORADOR AF' || $USER['rol']=='COLABORADOR UACI' || $USER['rol']=='COLABORADOR COMPRAS' || $USER['rol']=='ADMINISTRADOR SICBAF'){
