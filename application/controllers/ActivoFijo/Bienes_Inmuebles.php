    ...
    */
    ...
    if($USER){
      if ($this->User_model->validarAccesoCrud($modulo, $USER['id'], 'update')) {
        ...
        $this->table->set_heading('Id','Descripción','Código','Código anterior','Tipo_Inmueble','Extensión',
                                  'Zona','Dirección','Editar','Eliminar','Editar dato común');
        ...
        if (!($registros == FALSE)) {
          foreach($registros as $dat) {
              if($dat->terreno_zona!=NULL && $dat->tipo_inmueble!=NULL){
                ...
                '<a class="icono icon-actualizar" href="'.base_url('index.php/ActivoFijo/Datos_comunes/index/upgrade/'.$dat->id_dato_comun.'/'.$dat->id_bien).'"></a>');
              }
          }
        } else {
