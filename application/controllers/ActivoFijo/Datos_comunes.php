...
class Datos_comunes extends CI_Controller {
  ...
  public function __construct() {
    ...
    'ActivoFijo/Marcas_model','ActivoFijo/Doc_Ampara_Model', 'ActivoFijo/Cuenta_contable_model', 'ActivoFijo/Bienes_Muebles_Model'));
  }
  ...
  public function index(){
    ...
    $dc = array('dato_comun' => false);
    if ($this->uri->segment(4) == 'upgrade') {
      $dc['dato_comun'] = $this->Datos_Comunes_Model->obtenerDatoComunArray($this->uri->segment(5));
      $dc['id_bien'] = $this->uri->segment(6);
    }

    $data['body'] = $this->load->view('mensajes', $msg, TRUE) . $this->load->view('ActivoFijo/datos_comunes_view', $dc, TRUE) .
  ...
  */
  public function RecibirDatos(){
    ...
    if($USER){
        ...
        if (!($this->input->post('id_bien') == '')) {
          $bienes = $this->Datos_Comunes_Model->contieneBien($this->input->post('id'));
          if ($bienes->asociados <= 1) {
            if ($this->User_model->validarAccesoCrud($modulo, $USER['id'], 'update')) {
            ...
            if ($this->User_model->validarAccesoCrud($modulo, $USER['id'], 'insert')) {
              // Ingresa un nuevo dato comun
              $id_comun = $this->Datos_Comunes_Model->insertarDatosComunes($data);
              //asigna el nuevo dato comun al bien
              $this->Bienes_Muebles_Model->actualizarBien_mueble($this->input->post('id_bien'), array('id_dato_comun' => $id_comun, ));
