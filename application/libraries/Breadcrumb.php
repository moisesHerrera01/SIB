 ...
 class Breadcrumb {
     ...
     if ($breadcrump != '') {
       ...
       foreach ($breadcrump as $modulo) {
         ...
         if ($modulo['url_modulo'] == '') {
           ...
           $href = base_url("/index.php") . "/Submenu/index/" . $modulo['id_modulo'];

         } elseif ($modulo['url_modulo'] == '#') {

           $href = $modulo['url_modulo'];
   ...
   public function build_array_breadcrump($url) {
     ...
     if ($modulo) {
       ...
       array_push($breadcrump, array('id_modulo' => 0, 'dependencia' => 0, 'nombre_modulo' => 'Home', 'url_modulo' => 'dashboard'));
     }
   ...
   public function obtenerModulo($url) {
     $this->ci->db->select("a.id_modulo, a.dependencia, a.nombre_modulo, a.url_modulo")
          ->from("org_modulo a")
