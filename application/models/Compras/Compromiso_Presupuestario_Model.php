<?php
  class Compromiso_Presupuestario_Model extends CI_Model{
    ...
    public function obtenerCompromisosAutocomplete(){
      $this->db->select('c.id_compromiso,s.id_fuentes,f.nombre_fuente,c.id_orden_compra,c.numero_compromiso')
    ...
    public function esCompromisoBodega($solicitud){
      $this->db->from('sic_solicitud_compra s')
              ->join('sic_fuentes_fondo f','f.id_fuentes = s.id_fuentes')
              ->join('sic_detalle_solicitud_compra ds','ds.id_solicitud_compra=s.id_solicitud_compra')
              ->join('sic_detalle_producto dp','dp.id_detalleproducto=ds.id_detalleproducto')
              ->join('sic_especifico es','es.id_especifico=dp.id_especifico')
              ->order_by('s.id_solicitud_compra')
              ->where('es.proceso','BODEGA')
              ->where('s.id_solicitud_compra', $solicitud);
      $query = $this->db->get();
      if ($query->num_rows() == 1) {
          return TRUE;
      }
    ...
    public function obtenerCompromisoSolicitud($solicitud) {
      $this->db->select('c.id_compromiso,f.nombre_fuente,c.id_orden_compra,c.numero_compromiso')
              ->from('sic_compromiso_presupuestario c')
              ->join('sic_orden_compra o','o.id_orden_compra = c.id_orden_compra')
              ->join('sic_solicitud_compra s','s.id_solicitud_compra=o.id_solicitud_compra')
              ->join('sic_fuentes_fondo f','f.id_fuentes = s.id_fuentes')
      ...
      public function buscarCompromisos($busca,$anio,$rol,$empleado){
        $this->db->select('c.id_compromiso,c.numero_compromiso, s.id_fuentes,f.nombre_fuente,c.id_orden_compra,o.fecha,
        ...
        if ('COLABORADOR COMPRAS' == $rol) {
          $this->db->where('s.asignacion_solicitud', $empleado);
        }
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return  $query->result();
    ...
    public function totalCompromisos($anio,$rol,$empleado){
      if ('COLABORADOR COMPRAS' == $rol) {
        $this->db->where('s.asignacion_solicitud', $empleado);
      }
      $this->db->join('sic_orden_compra o','o.id_orden_compra = c.id_orden_compra')
               ->join('sic_solicitud_compra s','s.id_solicitud_compra=o.id_solicitud_compra')
               ->where("fecha_solicitud_compra BETWEEN '$anio-01-01' AND '$anio-12-31'");
      return $this->db->count_all('sic_compromiso_presupuestario c');
    }
    public function obtenerCompromisosLimit($porpagina, $segmento,$anio,$rol,$empleado){
      $this->db->select('c.id_compromiso,c.numero_compromiso,s.id_fuentes,f.nombre_fuente,c.id_orden_compra,o.fecha,
      o.id_solicitud_compra,o.numero_orden_compra,s.nivel_solicitud,s.estado_solicitud_compra,sd.id_solicitud_disponibilidad,
      s.numero_solicitud_compra,s.fecha_solicitud_compra')
              ->from('sic_compromiso_presupuestario c')
