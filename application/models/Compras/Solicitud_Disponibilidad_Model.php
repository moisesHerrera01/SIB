<?php
 class Solicitud_disponibilidad_model extends CI_Model{
  ...
  public $fecha_ingreso;
    ...
    public function existeSolicitudMonto($id) {
      $this->db->select('')
               ->from('sic_detalle_solicitud_disponibilidad dsd')
               ->join('sic_solicitud_disponibilidad sd','dsd.id_solicitud_disponibilidad=sd.id_solicitud_disponibilidad')
    ...
    public function totalSolicitudesDisponibilidad($anio,$rol,$empleado){
      $this->db->select('count(*) as total')
               ->from('sic_solicitud_disponibilidad sd')
               ->join('sic_solicitud_compra sc','sc.id_solicitud_compra=sd.id_solicitud_compra')
               ->where("sc.fecha_solicitud_compra BETWEEN '$anio-01-01' AND '$anio-12-31'");
      if ('COLABORADOR COMPRAS' == $rol) {
        $this->db->where('sc.asignacion_solicitud', $empleado);
      }
      $query=$this->db->get();
      $dispo=$query->row();
      return $dispo->total;
 ...
 public function obtenerSolicitudCompleta($id){
      ...
      public function buscarSolicitudesDisponibilidadLimit($busca,$anio,$rol,$empleado){
       ...
       sc.numero_solicitud_compra,sc.fecha_solicitud_compra,f.nombre_fuente')
                ->from('sic_solicitud_disponibilidad sd')
                ...
                ->like('sc.id_solicitud_compra',$busca)
                ->or_like('sd.id_solicitud_disponibilidad',$busca);
       if ('COLABORADOR COMPRAS' == $rol) {
         $this->db->where('sc.asignacion_solicitud', $empleado);
       }
       $query = $this->db->get();
     ...
     public function obtenerSolicitudesDisponibilidadLimit($porpagina,$segmento,$anio,$rol,$empleado){
       $this->db->select('sd.id_solicitud_disponibilidad,sd.id_solicitud_compra,sd.numero_confirmacion,sd.fecha,
    ...
    public function obtenerDisponibilidad($id_solicitud_compra){
     ...
     dsc.cantidad,sc.justificacion,sc.precio_estimado,l.linea_trabajo,sc.numero_solicitud_compra,sd.fecha_ingreso')
              ->from('sic_solicitud_disponibilidad sd')
              ->join('sic_solicitud_compra sc','sd.id_solicitud_compra=sc.id_solicitud_compra')
              ->join('sir_empleado em','em.id_empleado=sc.solicitante')
              ->join('sir_empleado_informacion_laboral e','e.id_empleado=em.id_empleado')
    ...
    public function obtenerSolicitudCompraAuto($nivel){
     $this->db->select('sc.numero_solicitud_compra,sc.id_solicitud_compra,dp.id_especifico,sec.nombre_seccion,sc.fecha_solicitud_compra')
   ...
   public function buscarSolicitudCompraAuto($nivel,$busca){
    $this->db->select('sc.id_solicitud_compra,sc.numero_solicitud_compra,dp.id_especifico,sec.nombre_seccion,sc.fecha_solicitud_compra')
             ->from('sic_solicitud_compra sc')
             ->join('sir_empleado em','em.id_empleado=sc.solicitante')
             ->join('sir_empleado_informacion_laboral e','e.id_empleado=em.id_empleado')
             ->join('org_seccion sec','sec.id_seccion=e.id_seccion')
             ...
             ->group_by('sc.id_solicitud_compra')
             ->order_by('sc.id_solicitud_compra desc')
             ->like('sc.numero_solicitud_compra',$busca)
             ->where('sc.nivel_solicitud',$nivel);
    $query = $this->db->get();
    if ($query->num_rows() > 0) {
        return  $query->result();
  ...
  public function reporteDisponibilidad($minFecha,$maxFecha,$segmento,$porpagina){
   ...
   sc.nivel_solicitud,sc.fecha_solicitud_compra')
            ...
            ->join('sic_fuentes_fondo f','f.id_fuentes=sc.id_fuentes','left')
 ...
 public function reporteDisponibilidadExcel($minFecha,$maxFecha){
  ...
  public function obtenerDescripcionProductos($id){
    $this->db->select('p.generico as nombre,det.cantidad,u.nombre as unidad,det.especificaciones')
             ->from('sic_solicitud_disponibilidad sd')
  ...
  public function obtenerSolicitudesDisponibilidadFiltro($fecha_inicio,$fecha_fin,$id_linea,$segmento,$porpagina){
      ...
      s.justificacion,o.monto_total_oc')
    ...
    public function obtenerSolicitudesDisponibilidadTotal($fecha_inicio,$fecha_fin,$id_linea){
        $this->db->select('o.id_orden_compra,o.numero_orden_compra,s.numero_solicitud_compra,f.nombre_fuente,o.fecha,
        sec.nombre_seccion,prov.nombre_proveedor,dsd.monto_sub_total,s.nivel_solicitud,s.fecha_solicitud_compra,s.justificacion,linea_trabajo,o.monto_total_oc')
                 ->from('sic_orden_compra o')
                 ->join('sic_proveedores prov','prov.id_proveedores=o.id_proveedores')
                 ->join('sic_solicitud_compra s','s.id_solicitud_compra=o.id_solicitud_compra')
                 ->join('sir_empleado e','e.id_empleado=s.solicitante')
      ...
      public function obtenerDisponibilidadAuto() {
        $this->db->select('a.id_solicitud_compra, b.id_detalle_solicitud_disponibilidad, c.linea_trabajo, b.monto_sub_total,sc.numero_solicitud_compra')
      ...
      public function verificarOrden($id_detalle_solicitud_disponibilidad){
        $this->db->select('*')
                 ->from('sic_orden_compra')
                 ->where('id_detalle_solicitud_disponibilidad',$id_detalle_solicitud_disponibilidad);
        $query=$this->db->get();
        if ($query->num_rows()>0) {
          return FALSE;
        }else {
