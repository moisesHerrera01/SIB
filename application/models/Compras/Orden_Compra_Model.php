<?php
  class Orden_compra_model extends CI_Model{
    ...
    public function obtenerOrdenComprasLimit($porpagina, $segment,$anio,$rol,$empleado) {
      $this->db->select('a.id_orden_compra, a.numero_orden_compra, a.fecha, a.observacion, a.lugar_notificaciones, b.id_proveedores, b.nombre_proveedor,
       ...
       sd.id_solicitud_disponibilidad,a.fecha,a.id_detalle_solicitud_disponibilidad,l.linea_trabajo,a.monto_total_oc, f.nombre_fuente,
       c.fecha_solicitud_compra,a.tipo,a.folio')
               ...
               ->limit($porpagina,$segment)
               ->where("c.fecha_solicitud_compra BETWEEN '$anio-01-01' AND '$anio-12-31'")
               ->order_by('id_orden_compra desc');
       if ('COLABORADOR COMPRAS' == $rol) {
         $this->db->where('c.asignacion_solicitud', $empleado);
       }
       $query = $this->db->get();
       if ($query->num_rows() > 0) {
           return  $query->result();
    ...
    public function buscarOrdenCompra($busca,$anio,$rol,$empleado) {
      $this->db->select('a.id_orden_compra, a.numero_orden_compra, a.fecha, a.observacion, a.lugar_notificaciones, b.id_proveedores, b.nombre_proveedor,
       c.numero_solicitud_compra,c.estado_solicitud_compra,c.nivel_solicitud,sd.monto_sub_total,c.id_solicitud_compra,
       sd.id_solicitud_disponibilidad,a.fecha,a.id_detalle_solicitud_disponibilidad,l.linea_trabajo,a.monto_total_oc, f.nombre_fuente,
       c.fecha_solicitud_compra,a.tipo,a.folio')
               ...
               ->join('org_linea_trabajo l','l.id_linea_trabajo=sd.id_linea_trabajo')
               ->where("c.fecha_solicitud_compra BETWEEN '$anio-01-01' AND '$anio-12-31'")
               ->like('c.id_solicitud_compra',$busca)
               ->order_by('id_orden_compra desc');
       if ('COLABORADOR COMPRAS' == $rol) {
         $this->db->where('c.asignacion_solicitud', $empleado);
       }
    ...
    public function totalOrdenes($anio,$rol,$empleado){
  ...
  public function obtenerOrdenesFiltro($tipo,$fecha_inicio,$fecha_fin,$segmento,$porpagina){
               ...
               if($tipo==NULL){
                 ...
                 o.fecha,sec.nombre_seccion,p.generico as nombre,prov.nombre_proveedor,o.monto_total_oc,det.cantidad,catp.tipo_empresa,
                 s.nivel_solicitud,s.fecha_solicitud_compra,s.justificacion,o.folio')
                          ->from('sic_orden_compra o')
                          ...
                          ->join('sic_fuentes_fondo f','f.id_fuentes=s.id_fuentes','left')
                 ...
                 sec.nombre_seccion,p.generico as nombre,prov.nombre_proveedor,o.monto_total_oc,det.cantidad,catp.tipo_empresa,
      ...
      public function obtenerOrdenesExcel($tipo,$fecha_inicio,$fecha_fin){
        if($tipo==NULL){
          ...
          s.nivel_solicitud,s.fecha_solicitud_compra,s.justificacion,o.monto_total_oc,o.folio')
                   ->from('sic_orden_compra o')
                   ->join('sic_proveedores prov','prov.id_proveedores=o.id_proveedores')
                   ->join('sic_categoria_proveedor catp','catp.id_categoria_proveedor=prov.id_categoria_proveedor')
                   ->join('sic_solicitud_compra s','s.id_solicitud_compra=o.id_solicitud_compra')
        ...
        public function existeDetalleResumen($id) {
          $this->db->select('')
                   ->from('sic_orden_compra o')
                   ->join('sic_detalle_orden_resumen or','or.id_orden_compra=o.id_orden_compra')
                   ->where('or.id_orden_compra',$id);
          $query=$this->db->get();
          if ($query->num_rows()>0){
            return TRUE;
          } else {
