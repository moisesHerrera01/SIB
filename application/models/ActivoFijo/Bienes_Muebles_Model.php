<?php
  class Bienes_muebles_model extends CI_Model{
    ...
    public function insertarBienes_muebles($data){
        ...
        $id_bien = $this->db->insert_id();

        $data_movimiento = array(
            'id_oficina_entrega' => 3,
            'id_oficina_recibe' => $data['id_oficina'],
            'id_seccion' => $this->obtenerSeccionOficina($data['id_oficina'])->id_seccion,
    ...
    public function insertarBienesCarga($data) {
      $this->id_dato_comun = $data['id_dato_comun'];
        $this->codigo_anterior = $data['codigo_anterior'];
        $this->serie = $data['serie'];
        $this->numero_motor = $data['numero_motor'];
    ...
    public function buscarBienes_muebles($user,$busca){
      $this->db->select('a.id_bien, e.codigo_anterior, e.codigo, e.serie, f.descripcion, c.id_oficina, c.nombre_oficina,
  		d.primer_nombre, d.primer_apellido, d.id_empleado, f.precio_unitario, g.nombre_marca, f.modelo, f.id_dato_comun,
      e.numero_motor, e.numero_placa, e.matricula, e.observacion')
              ->from('(
              ...
              ->join('sic_bien e', 'e.id_bien = a.id_bien')
              ->join('sic_datos_comunes f', 'f.id_dato_comun = e.id_dato_comun')
              ->join('sic_subcategoria sub','sub.id_subcategoria=f.id_subcategoria')
              ->join('sic_marcas g', 'g.id_marca = f.id_marca')
              ->order_by('e.id_bien','desc')
              ->where('e.terreno_zona=',NULL)
              ->where('e.tipo_inmueble=',NULL)
              ...
              if ($user=='TECNICO UDT') {
                ...
                $this->db->where_in('sub.nombre_subcategoria',$names);
              }
      $query = $this->db->get();
      if ($query->num_rows() > 0) {
          return  $query->result();
    ...
    public function actualizarBien_mueble_Cerrar($id, $id_condicion_bien){
      $data = array('id_condicion_bien' => $id_condicion_bien);
    ...
    public function obtenerBienes_mueblesLimit($porpagina, $segmento,$user){
      $this->db->select('a.id_bien, e.codigo_anterior, e.codigo, e.serie, f.descripcion, c.id_oficina, c.nombre_oficina,
  		d.primer_nombre, d.primer_apellido, d.id_empleado, f.precio_unitario, g.nombre_marca, f.modelo, f.id_dato_comun,
      e.numero_motor, e.numero_placa, e.matricula, e.observacion')
              ->from('(
    ...
    public function totalBienes_muebles($user){
    ...
    public function obtenerSeccionOficina($oficina) {
      $this->db->from('org_oficina a')
               ->join('org_seccion_has_almacen b', 'a.id_seccion_has_almacen = b.id_seccion_has_almacen')
               ->where('a.id_oficina', $oficina);
      $query=$this->db->get();
