<?php
  class Datos_comunes_model extends CI_Model{
    ...
    public function insertarDatosComunes($data){
        ...
        if (array_key_exists('id_dato_comun', $data)) {
    ...
    public function obtenerDatoComunArray($id) {
      $this->db->select('d.id_dato_comun, d.id_subcategoria, d.id_tipo_movimiento, d.id_marca, d.descripcion, d.modelo,
                        d.color, d.id_doc_ampara, d.nombre_doc_ampara as doc_ampara, d.fecha_adquisicion, d.precio_unitario, d.id_proveedores,
                        d.id_fuentes, d.garantia_mes, d.observacion, d.id_cuenta_contable, s.nombre_subcategoria, m.nombre_marca,
                        dc.nombre_doc_ampara, pv.nombre_proveedor, py.nombre_fuente, c.nombre_cuenta')
  ...
  public function buscarBienesAutocomplete($buscar){
     ...
     public function buscarPorCualquierCampo($criterio,$porpagina,$segmento){
        ...
        d.fecha_adquisicion,d.precio_unitario,d.id_proveedores,pv.nombre_proveedor,d.id_fuentes,py.nombre_fuente,d.garantia_mes,
        d.observacion,d.id_cuenta_contable,c.nombre_cuenta,d.codificar,cat.nombre_categoria,b.id_bien,b.codigo,b.serie,b.numero_motor,
        b.numero_placa,b.matricula,cb.nombre_condicion_bien,b.codigo,b.codigo_anterior,of.nombre_oficina,emp.nombre_empleado,cat.numero_categoria,m.id_marca,c.id_cuenta_contable')
                 ->from("(select max(id_detalle_movimiento) as id_detalle_movimiento,max(id_movimiento) as id_movimiento,
                  id_bien from sic_detalle_movimiento group by id_bien order by id_detalle_movimiento) as dm")
                 ->join('sic_movimiento mov','dm.id_movimiento=mov.id_movimiento')
                 ->join('sic_bien b','b.id_bien=dm.id_bien')
                 ...
                 ->join('sic_condicion_bien cb','cb.id_condicion_bien=b.id_condicion_bien')
                 ->join('org_oficina of','of.id_oficina=mov.id_oficina_recibe')
                 ->join("(select id_empleado,CONCAT_WS(' ',primer_nombre,segundo_nombre,primer_apellido,segundo_apellido)
                  as nombre_empleado from sir_empleado) as emp",'emp.id_empleado=mov.id_empleado')
                 ->join('sic_subcategoria s', 's.id_subcategoria = d.id_subcategoria')
                 ->join('sic_categoria cat','cat.id_categoria=s.id_categoria')
                 ->join('sic_tipo_movimiento t', 't.id_tipo_movimiento = mov.id_tipo_movimiento')
                 ->join('sic_marcas m', 'm.id_marca = d.id_marca')
                 ...
                 $this->db->like('b.id_bien',$criterio)
                         ...
                         ->or_like('emp.nombre_empleado',$criterio);
               }
                 $this->db->limit($porpagina,$segmento)
                 ->where('mov.estado_movimiento','CERRADO');
      $query = $this->db->get();
   ...
   public function buscarPorCualquierCampoAutocomplete($criterio){
      $this->db->select(
              'd.id_dato_comun,
               ...
               d.color,
               d.id_doc_ampara,
               d.nombre_doc_ampara as numero_doc,
               d.fecha_adquisicion,
               d.precio_unitario,
               d.id_proveedores,
               d.id_fuentes,
               ...
               s.numero_subcategoria,
               t.nombre_movimiento,
               m.nombre_marca,
               m.id_marca,
               dc.nombre_doc_ampara,
               dc.id_doc_ampara,
               pv.nombre_proveedor,
               pv.id_proveedores,
               py.nombre_fuente,
               ...
               cat.numero_categoria,
               cb.nombre_condicion_bien,
               of.nombre_oficina,
               of.id_oficina,
               alm.nombre_almacen,
               sec.nombre_seccion,
               ...
               ->join('sic_condicion_bien cb','cb.id_condicion_bien=b.id_condicion_bien')
               ->join('org_oficina of','of.id_oficina=mov.id_oficina_recibe')
               ->join('org_seccion_has_almacen sha','sha.id_seccion_has_almacen=of.id_seccion_has_almacen')
               ->join('org_seccion sec','sec.id_seccion=sha.id_seccion')
               ->join('org_almacen alm','alm.id_almacen=sha.id_almacen')
               ->join("(select id_empleado,CONCAT_WS(' ',primer_nombre,segundo_nombre,primer_apellido,segundo_apellido)
                as nombre_empleado from sir_empleado) as emp",'emp.id_empleado=mov.id_empleado')
               ->join('sic_subcategoria s', 's.id_subcategoria = d.id_subcategoria')
               ->join('sic_categoria cat','cat.id_categoria=s.id_categoria')
