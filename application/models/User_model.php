<?php
  class User_model extends CI_Model{
   ...
   public function obtenerModuloCompleto($id_modulo) {
     $this->db->select('id_modulo, nombre_modulo, url_modulo, img_modulo')
             ->from('org_modulo')
   ...
   public function obtenerEmpleado($empleado) {
     $this->db->where("id_empleado", $empleado);
     $this->db->join("org_usuario b", 'a.nr_empleado = b.nr');
     $query = $this->db->get('Lista_empleados_estado a');
     if ($query->num_rows() > 0) {
         return  $query->row();
     }
     else {
   ...
   public function obtenerColaboradoesCompra($id_rol, $busca = '') {
     $this->db->select('*')
             ->from('org_usuario a')
             ->join('org_usuario_rol b', 'a.id_usuario = b.id_usuario')
             ->join('Lista_empleados_estado e','a.nr=e.nr_empleado')
